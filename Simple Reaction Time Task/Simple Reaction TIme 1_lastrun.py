...
expName = 'Simple Reaction TIme 1'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\PSY310\\Simple Reaction Task\\Simple Reaction TIme 1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # create an object to store info about Routine instruction_
    instruction_ = data.Routine(
        name='instruction_',
        components=[text, key_resp_2],
    )
    instruction_.status = NOT_STARTED
    ...
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # store start times for instruction_
    instruction_.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instruction_.tStart = globalClock.getTime(format='float')
    instruction_.status = STARTED
    thisExp.addData('instruction_.started', instruction_.tStart)
    instruction_.maxDuration = None
    # keep track of which components have finished
    instruction_Components = instruction_.components
    for thisComponent in instruction_.components:
    ...
    thisExp.currentRoutine = instruction_
    instruction_.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if not continueRoutine:
            instruction_.forceEnded = routineForceEnded = True
        # has the Routine been forcibly ended?
        if instruction_.forceEnded or routineForceEnded:
            break
    ...
    # store stop times for instruction_
    instruction_.tStop = globalClock.getTime(format='float')
    instruction_.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instruction_.stopped', instruction_.tStop)
    # check responses
